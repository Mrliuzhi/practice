...
myprot_rqueue::myprot_rqueue() {
  head_ = tail_ = 0;
  len_ = 0;
  limit_ = MYPROT_RTQ_MAX_LEN;
  timeout_ = MYPROT_RTQ_TIMEOUT;
}

void
myprot_rqueue::enque(Packet *p) {
...
Packet*
myprot_rqueue::deque(nsaddr_t dst) {
Packet *p, *prev;
...
char 
myprot_rqueue::find(nsaddr_t dst) {
Packet *p, *prev;  
...
void
myprot_rqueue::findPacketWithDst(nsaddr_t dst, Packet*& p, Packet*& prev) {
...
bool
myprot_rqueue::findAgedPacket(Packet*& p, Packet*& prev) {
